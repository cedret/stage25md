...
#        echo "            gateway4: 192.168.80.2" | sudo tee -a "$cible2"
        ...
        sudo hostnamectl set-hostname tst20prod
...
#        echo "            gateway4: 192.168.80.2" | sudo tee -a "$cible2"
        ...
        echo "              addresses:" | sudo tee -a "$cible2"
        echo "                  - 192.168.80.2" | sudo tee -a "$cible2"
        sudo hostnamectl set-hostname tst20rvpx
        echo -e "${vert} Vérifier et appliquer ATTENTION CHANGEMENT IP !!!"
        echo -e "${jaune} sudo nano /etc/hosts"
        echo -e "${jaune} sudo netplan apply"
        ;;
...
while true;
    ...
    echo -e "${blanc}11 -> Config: RESEAU           12 -> VERSIONS actives               13 -> FLUX actifs"
    echo -e "${rouge}21 -> Install PROD: Apache2    22 -> Install: PROD A/T/PG           23 -> Install RVPX"
    echo -e "${rouge}24 -> Install SS-SSL PROD      25 -> Install SS-SSL RVPX "
    echo -e "${rouge}26 -> Install IpSec PROD       27 -> Install IpSec RVPX "
    echo -e "${gris}31 -> Afficher config SS-SSL   32 -> Afficher config IPsec          33 -> Afficher repere.lst"
    echo -e "${blanc}34 -> Verif SSL                35 -> Verif IPsec                    "
    echo -e "${gris}36 -> Start/ stop SSL          37 -> Start/ stop IPsec              38 -> Lynx"
    echo -e "${blanc}41 -> Eteindre serveur         42 -> Editer ce script               43 -> MAJ Scripts???"
    echo -e "${gris}44 -> SOS"
    ...
    case $reponse in
      ...
      31)
        ...
        echo -e "${jaune}========== cat /etc/ssl/prod/prod.key${blanc}"
        sudo cat /etc/ssl/prod/prod.key
        echo -e "${jaune}========== cat /etc/ssl/prod/prod.crt${blanc}"
